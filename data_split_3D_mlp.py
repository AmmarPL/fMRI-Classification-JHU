...
def split_data(data_dir, directory_label):
    ...
    for nii in sorted(data_dir.glob('dr_stage2_ic*nii.gz')):
        ...
        nii_img.append([nii,image])  # We need to append a tuple here (nii,image) nii_img.append(image) 

    print("Length of nii list is (should be 100 [clusters]): ",len(nii_img))
    # print("nii image filename is (should be 100 [clusters]): ",nii_img[0][0])
    print("Shape of each image is (last should be 176 [people]: ",nii_img[0][1].shape) #[0][1] print("Shape of each image is (last should be 176 [people]: ",nii_img[0].shape)
    ...
    #
    all_indices = list(range(nii_img[0][1].shape[-1])) #all_indices = list(range(nii_img[0].shape[-1]))
    ...
    tt, test_indices = train_test_split(all_indices, test_size=nii_img[0][1].shape[-1]-2, shuffle = True, random_state=42) #Everyyting else
    ...
    # subjects_test = create_subjects_list(nii_img, test_indices, labels) #Beijing
    subjects_test = create_subjects_list(nii_img, list(range(nii_img[0][1].shape[-1])), labels) #everything else
...
def create_subjects_list(nii_img, patient_indices, labels):
    ...
    subjects = []
    # print("filenames are",nii_img[0][0])
    print(nii_img[1][0])
    for patient_index in patient_indices: 
        for cluster_index in range(100):
            subjects += [tio.Subject(
                volume = tio.ScalarImage(tensor=torch.Tensor(nii_img[cluster_index][1][:,:,:,patient_index][None,])), 
                label = labels[cluster_index] ,
                filename = str(nii_img[cluster_index][0]),
            )]  #nii_img[:1]
...
def create_subjects_dict(nii_img, patient_indices, labels):
    ...
    for patient_index in patient_indices: 
        for cluster_index in range(100):
            subjects.append(nii_img[cluster_index][1][:,:,:,patient_index])
